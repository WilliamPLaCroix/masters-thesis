...
RUN_KEY="${PROJECT_VERSION}-ots-base-model-${ITERATION}"
...
for GRADE in "${GRADES[@]}"; do
    ...
    OUT_ADAPTER="${CACHE}/${PROJECT_VERSION}_ots_base_model_grade@${TARGET_GRADE}"
    # mkdir -p "${OUT_ADAPTER}"
    ...
    # ---------------- Core W&B env ----------------
    export WANDB_PROJECT="Thesis_Phase_${PROJECT_VERSION}.4" ### TODO CHANGE BACK "Thesis_Phase_${PROJECT_VERSION}", we need to hack in the .4 for now
    ...
    export WANDB_NAME="ots_base_model"
    ...
    llamafactory-cli train \
      ...
      --report_to wandb \
      --run_name "${WANDB_NAME}" \
      > "${LOG_DIR}/ots_base_model_grade${TARGET_GRADE}_eval.log" 2>&1
    # -------------- INFERENCE END --------------
